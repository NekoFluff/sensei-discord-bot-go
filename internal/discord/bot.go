...
func NewBot(token string, log *logrus.Entry) *Bot {
...
func (bot *Bot) RegisterCommands() {
	for _, cmd := range bot.Commands {
		...
		if err != nil {
			bot.log.Panicf("Cannot create '%v' command: %v", cmd.Name, err)
		}
	}
}
...
func (bot *Bot) SendChannelMessage(channelName string, message string) {
	for _, guild := range bot.Session.State.Guilds {
		...
		for _, c := range channels {
			...
			if err != nil {
				bot.log.Println("An error occurred while sending a message to a discord server")
				bot.log.Println(err)
			}
		}
	}
